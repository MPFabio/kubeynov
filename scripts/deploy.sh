...
echo "Deploiement de la plateforme DevOps..."

# Vérifier que le cluster existe
if ! kubectl cluster-info &> /dev/null; then
    echo "ERREUR: Aucun cluster Kubernetes trouvé. Executez d'abord ./scripts/setup-kind.sh"
    exit 1
fi
...
docker build --progress=plain --no-cache=false -t backend:latest . || {
    echo "ERREUR: Erreur lors du build de l'image backend"
    exit 1
}
...
docker build --progress=plain --no-cache=false -t frontend:latest . || {
    echo "ERREUR: Erreur lors du build de l'image frontend"
    exit 1
}
...
echo "Deploiement de PostgreSQL..."
